...
[前回の記事](https://reotech736.com/2026/08/10/yolo26-scale-resolution-comparison.html)では、YOLO26n・s・mを320pxと640pxで組み合わせた6条件をFP32で比較し、量子化比較へ渡す候補として **YOLO26n（640px）とYOLO26s（640px）** を残しました。
...
| [量子化](https://reotech736.com/terms/model-quantization/) | FP32の値を限られた整数範囲へ写す変換 |
| [static quantization](https://reotech736.com/terms/static-quantization/) | 事前に代表画像を流し、推論途中の値の範囲を固定する方式 |
| [calibration](https://reotech736.com/terms/quantization-calibration/) | 代表画像からscaleとzero pointを決める工程。学習ではない |
| scale / zero point | 整数1段分の幅と、実数0に対応する基準点 |
| [QDQ](https://reotech736.com/terms/qdq/) | QuantizeLinear / DequantizeLinearをONNX graphへ挿入する表現 |
| [AP 50-95](https://reotech736.com/terms/average-precision/) | 見逃しと誤検出を複数のIoU条件でまとめた品質指標。高いほど良い |
| [quantization drift](https://reotech736.com/terms/quantization-drift/) | FP32からINT8へ変えたことで生じる[bbox](https://reotech736.com/terms/bounding-box/)、confidence、検出件数などの差 |
| p50 / p95 | 測定値の中央値と、95%の値が収まる境界 |
| model size | 保存したONNXファイルの容量 |
| [working set](https://reotech736.com/terms/working-set/) | 実行processが物理メモリ上で使用する領域 |
...
![同じONNX Runtime CPU backendで比較できるFP32とINT8を残し、FP16を今回の検証候補として棄却した判断図](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/fp16-comparison-scope.svg)
...
今回は[COCO](https://reotech736.com/terms/coco-dataset/) train2017からseed 17で固定した512画像を、nとsで共通のcalibration用画像にしました。評価結果を見て画像や方式を選び直していません。

![COCO train2017の512画像でscaleとzero pointを決め、別のCOCO val2017全5000画像でFP32とINT8の品質を評価する流れ](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/static-quantization-flow.svg)
...
| [レイテンシ](https://reotech736.com/terms/latency/) | 固定100画像、confidence 0.4、warm-up 50 |
...
![YOLO26nとsのFP32・INT8について、平均AP、PC ORT推論p50、ONNX容量を比較した3パネル図](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/fp32-int8-quality-speed-size.png)
...
![YOLO26nとsをFP32からINT8へ変えたとき、ONNX容量は減る一方で品質が下がりPC推論時間が増えたことを示す散布図](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/n-s-pareto.svg)

*図4: INT8化によるtrade-off*

[パレートフロンティア](https://reotech736.com/terms/pareto-frontier/)に残っても、最低品質を満たすとは限りません。quality gateを先に適用すると、Androidへ渡せるのはn・FP32だけでした。

## 小さくなっても速くならなかった構造

![FP32は384 node、INT8は1000 nodeで、QuantizeLinear 206、DequantizeLinear 410、量子化対象106、FP32のままのnode 278がある構成図](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/graph-node-growth.svg)
...
![YOLO26nとsのFP32・INT8を5回ずつ測り、どちらもINT8の推論時間範囲がFP32と重ならなかった結果](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/latency-repeat-ranges.svg)
...
![海辺でヨットを準備する人々の同一画像に対し、YOLO26n FP32とINT8のperson検出を左右で比較した例](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/detection-example-yolo26n-fp32-int8-coco-000000078565.jpg)
...
![nとsのINT8は容量削減に成功したが品質gateに不合格となり、POCO X8 ProへはYOLO26n（640px・FP32）を渡す判断図](https://reotech736.com/assets/images/posts/yolo26-fp32-static-int8-comparison/selection-result.svg)
...
- [前回記事：YOLO26のモデル規模・入力解像度による性能差を比較した](https://reotech736.com/2026/08/10/yolo26-scale-resolution-comparison.html)
...
この記事は[Reo's Tech Blogの同名記事](https://reotech736.com/2026/08/10/yolo26-fp32-static-int8-comparison.html)にも掲載しています。
