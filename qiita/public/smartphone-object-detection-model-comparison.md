...
| 用語 | この記事での意味 |
| --- | --- |
| [物体検出](https://reotech736.com/terms/object-detection/) | 画像の中から「人」「車」などを見つけ、種類と場所を示す処理 |
| [bbox（バウンディングボックス）](https://reotech736.com/terms/bounding-box/) | 見つけた物体の場所を囲む四角い枠 |
| [ONNX](https://reotech736.com/terms/onnx/) | AIモデルを別の環境へ受け渡すための共通ファイル形式 |
| [ONNX Runtime（ORT）](https://reotech736.com/terms/onnx-runtime/) | ONNX形式のモデルをPCやスマートフォンで動かすソフトウェア |
| [COCO](https://reotech736.com/terms/coco-dataset/) | 人や車の位置を正解データとして持つ、評価用にも使われる画像集 |
| [IoU](https://reotech736.com/terms/intersection-over-union/) | 予測した枠と正解の枠がどれくらい重なるかを0〜1で表す値 |
| [AP 50-95](https://reotech736.com/terms/average-precision/) | 見逃しと誤検出のバランスをまとめた品質指標。大きいほど良い |
| [入力解像度](https://reotech736.com/terms/input-resolution/) | モデルへ渡す画像の縦横サイズ。今回は320×320 |
| [モデル量子化](https://reotech736.com/terms/model-quantization/) | 数値をINT8などで表し、モデルの小型化や高速化を狙う変換 |
| [スループット](https://reotech736.com/terms/throughput/) | 1秒間に処理できる画像数など、一定時間内の処理量 |
...
![YOLO26n、D-FINE-N、PicoDet-Sから利用条件を確認し、共通320pxでONNX一致、COCO品質、PC速度を比較した流れ](https://reotech736.com/assets/images/posts/smartphone-object-detection-model-comparison/evaluation-flow.svg)
...
![YOLO26nとD-FINE-Nについて、変換前のframeworkとONNX Runtimeの全検出が一致したことを示す集計図](https://reotech736.com/assets/images/posts/smartphone-object-detection-model-comparison/framework-ort-parity.png)
...
![COCO val2017全5,000画像におけるYOLO26nとD-FINE-NのpersonとcarのAP 50-95を比較した棒グラフ](https://reotech736.com/assets/images/posts/smartphone-object-detection-model-comparison/quality-coco-ap50-95.png)
...
![固定500画像で測ったbbox下端中央のp95 d2を比較し、0.08の基準線を示した棒グラフ](https://reotech736.com/assets/images/posts/smartphone-object-detection-model-comparison/quality-detection-point-p95-d2.png)
...
![Windows PCのCPU上で測ったYOLO26nとD-FINE-NのframeworkおよびONNX Runtime推論時間のp50とp95を比較した棒グラフ](https://reotech736.com/assets/images/posts/smartphone-object-detection-model-comparison/latency-inference-p50-p95.png)
...
![次回にYOLO26n、s、mの320と640を比較し、次々回に選定モデルのFP32、FP16、INT8を比較するロードマップ](https://reotech736.com/assets/images/posts/smartphone-object-detection-model-comparison/future-evaluation-roadmap.svg)
...
この記事は[Reo's Tech Blogの同名記事](https://reotech736.com/2026/08/10/smartphone-object-detection-model-comparison.html)にも掲載しています。
