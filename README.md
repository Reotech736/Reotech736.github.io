...
`update-contributions.yml` は、直前の `main` の先頭 commit が `chore: update contributions svg` の bot commit で、変更対象も `assets/images/contributions.svg` だけだった場合は `--amend` で上書きします。
そのため、この自動更新 commit が連続したときは履歴上 1 つにまとまります。
